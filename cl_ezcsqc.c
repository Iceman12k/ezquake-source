...
static void R_CSQC_MissileTrail(vec3_t start, vec3_t end, vec3_t angles, int trail_num)
{
	...
	if (trail_num == 1) {
		...
		FireballTrail(start, end, color, 2, 0.5);
	}
	else if (trail_num == 9) {
	...
	if (self->modelflags & EF_ROCKET)
	{
		...
		if (rocketlightsize >= 1) {
			...
			if ((r_rockettrail.value < 8 || r_rockettrail.value == 12) || !gl_part_trails.integer) {
				...
				CL_NewDlightEx(self->entnum, self->origin, rocketlightsize, 0.1, &cst_lt, bubble);
			...
			if (r_rockettrail.value == 9 || r_rockettrail.value == 11) {
				CL_NewDlight(self->entnum, self->origin, rocketlightsize, 0.1, lt_default, bubble);
			}
			else if (r_rockettrail.value == 8) {
				// PLASMA ROCKETS
				CL_NewDlight(self->entnum, self->origin, rocketlightsize, 0.1, lt_blue, bubble);
			}
			else if (r_rockettrail.value == 10) {
				// FUEL ROD GUN
				CL_NewDlight(self->entnum, self->origin, rocketlightsize, 0.1, lt_green, bubble);
			}
		}
		...
		if (amf_nailtrail.integer)
		{
			...
			if (amf_nailtrail_plasma.integer)
			{
				if (dist > 3)
				{
					byte color[3];
					color[0] = 0; color[1] = 70; color[2] = 255;
					FireballTrail(self->trail_origin, self->origin, color, 0.6, 0.3);
			...
			else
			{
				if (dist > 14)
				{
					ParticleNailTrail_NoEntity(self->trail_origin, self->origin, 2, 0.014f);
...
void EntUpdate_WeaponDef(ezcsqc_entity_t *self, qbool is_new)
{
	...
	//Con_Printf("EntUpdate_WeaponDef %i\n", sendflags);
...
void WeaponPred_Simulate(usercmd_t u, player_state_t ps, ezcsqc_weapon_state_t *ws)
{
	...
	{
		ws->impulse = 0;
		ws->attack_finished = ws->client_time + 0.05;
		return;
	}

	ws->client_time += u.msec * 0.001;
...
qbool CL_EZCSQC_Event_Sound(int entnum, int channel, int soundnumber, float vol, float attenuation, vec3_t pos, float pitchmod, float flags)
{
	if (entnum == cl.playernum + 1)
	{
		...
		for (weppredsound_t *snd = predictionsoundlist; snd; snd = snd->next)
		{
			if (snd->chan != channel)
				continue;
