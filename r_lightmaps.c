...
static int R_LightmapSurfaceSortFunction(const void* lhs_, const void* rhs_)
{
	...
	if (r_lightmap_packbytexture.integer == 1) {
		...
		int rhs_size = ((rhs->extents[0] >> lhs->lmshift) + 1) * ((rhs->extents[1] >> lhs->lmshift) + 1);
		...
		int rhs_width = ((rhs->extents[0] >> lhs->lmshift) + 1);
		int rhs_height = ((rhs->extents[1] >> lhs->lmshift) + 1);
